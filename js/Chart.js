...
(function(){
	...
	var each = helpers.each = function(loopable,callback,self){
		...
		template = helpers.template = function(templateString, valuesObject){
			...
			var cache = {};function tmpl(str, data){
				...
				var returnvalue = data ? fn( data ) : fn;
				return returnvalue;
			}
			...
			if (ChartElements.length > 0){
				...
				if (this.datasets && this.datasets.length > 1) {
					...
					each(ChartElements, function(Element) {
						...
						var what = JSON.parse(JSON.stringify(Element));
						what.value = Element.value.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,")+'원';
						new Chart.Tooltip({
							...
							text: template(this.options.tooltipTemplate, what),
							chart: this.chart,
							custom: this.options.customTooltips
						}).draw();
					}, this);
	...
	Chart.MultiTooltip = Chart.Element.extend({
		initialize : function(){
			...
			{
				this.labels[index] = this.labels[index] + ' (' + this.names[index] + ')';
			}
	...
	Chart.Type.extend({
		...
		buildScale : function(data){
			this.scale = new Chart.RadialScale({
				...
				backdropPaddingY : this.options.scaleBackdropPaddingY,
				backdropPaddingX: this.options.scaleBackdropPaddingX,
				lineWidth: (this.options.scaleㅇShowLine) ? this.options.scaleLineWidth : 0,
				lineColor: this.options.scaleLineColor,
